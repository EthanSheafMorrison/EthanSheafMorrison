...
export default function FeaturedCarousel({ items }: { items: FeaturedItem[] }) {
  ...
  return (
    <section className="h-full flex flex-col overflow-hidden">
      <div className="site-grid py-6">
        <div className="col-span-12 flex items-center justify-between">
          <h2 className="text-2xl md:text-3xl font-semibold">Featured</h2>
          <div className="flex gap-2">
            <button className="border px-3 py-0 focus:outline-none" onClick={prev} aria-label="Previous">←</button>
            <button className="border px-3 py-1 focus:outline-none" onClick={next} aria-label="Next">→</button>
          </div>
        </div>
      </div>
        ...
        >
          {list.map((it) => (
            <div key={it.slug} className="w-full shrink-0 h-full min-h-0">
              <div className="site-grid items-center h-full min-h-0 py-8">
                ...
                <Link href={`/projects/${it.slug}`} className="col-span-12 md:col-span-6 order-1 md:order-2 block">
                  <div className="relative h-[45vh] md:h-[55vh] overflow-hidden bg-white">
                    {it.cover ? (
